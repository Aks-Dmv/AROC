...
from train.ACAv import trainac
from test.AC import testac
...
if __name__ == '__main__':
    ...
    env = OC_env(args.env)
    shared_model = ACModel(env.observation_space.shape[0], env.action_space,args.options,args.width)
    if args.load:
    ...
    p = mp.Process(target=testac, args=(args, shared_model, env_conf))
    p.start()
    processes.append(p)
    time.sleep(0.1)
    for rank in range(0, args.workers):
        p = mp.Process(
            target=trainac, args=(rank, args, shared_model, optimizer, env_conf))
        p.start()
        processes.append(p)
        time.sleep(0.1)
    for p in processes:
