...
from train.OCPGAv import trainocpg
from test.OCPG import testocpg
from shared_optim import SharedRMSprop, SharedAdam
#from gym.configuration import undo_logger_setup
import time

#undo_logger_setup()
parser = argparse.ArgumentParser(description='OCPG')
parser.add_argument(
    '--lr',
    type=float,
    default=0.0001,
...
if __name__ == '__main__':
    ...
    env = OC_env(args.env)
    shared_model = OCPGModel(env.observation_space.shape[0], env.action_space,args.options,args.width)
    if args.load:
    ...
    if args.shared_optimizer:
        ...
        if args.load:
            saved_state = torch.load('{0}{1}1.torch'.format(args.load_model_dir, args.env),map_location=lambda storage, loc: storage)
    ...
    for rank in range(0, args.workers):
        p = mp.Process(
            target=trainocpg, args=(rank, args, shared_model, optimizer, env_conf))
        p.start()
        processes.append(p)
        time.sleep(0.1)
    for p in processes:
